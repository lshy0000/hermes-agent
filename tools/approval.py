...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(curl|wget)\b.*\|\s*(python[23]?|perl|ruby|node|php)\b', "pipe remote content to interpreter"),
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # Skip tirith if the matching dangerous pattern is already permanently
    # approved — this prevents tirith from re-triggering approval prompts
    # for commands the user has explicitly allowlisted.
    if tirith_result["action"] in ("block", "warn") and not (
        is_dangerous and is_approved(session_key, pattern_key)
    ):
        findings = tirith_result.get("findings") or []
